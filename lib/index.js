...
(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        // Browser globals
        //Import with script tag: momentjs, jquery and angularjs.
        //Necessary bootstrap to render.
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                if (!scope.env && process && process.hasOwnProperty("env") && process.env['NODE_ENV']) {
                    scope.env = (process.env['NODE_ENV'].toLowerCase() == "dev" 
                                    || process.env['NODE_ENV'].toLowerCase() == "development") ? "DEV" : "OTHER";
                } else if (!scope.env) {
                    scope.env = "PROD";
                }
                scope.env = "PROD";
                ...
                function formatDate(date) {
                    var locale = scope.locale || scope.options.locale;
                ...
                function updateModelValue() {
                    var date = {
                        startDate: scope.startDate ? convertDate(scope.startDate) : null,
                        endDate: scope.endDate ? convertDate(scope.endDate) : null,
                    };
                ...
                function updateModel() {
                    scope.options.startDate = moment().subtract(1,'days')//getStartDate();
                    scope.options.endDate = moment()//getEndDate();
                    scope.options.minDate = getMinDate();
                    scope.options.maxDate = getMaxDate();//moment().add(1,'days').format('MM/DD/YYYY');
                    ...
                    scope.options.ranges = {
                        ...
                        'Past Year': [moment().subtract(365, 'days'), moment()],
                        'All Time': [moment().subtract(1,'days'),moment()]
                    };
                    scope.options.opens = 'right';
                ...
                function getOptions() {
                    scope.options = {
                        ...
                        minDate: getMinDate(),
                        maxDate: getMaxDate(),
                        locale: scope.locale ? scope.locale : {
                ...
                (function () {
                    ...
                    if (scope.env == "DEV")
                        $log.info("%cSM DATE-RANGE-PICKER v1.0.2", $logger.properties.style.abbr,
                                    "\dateFormat : " + scope.locale.format + "\n" +
                ...
                function createElementDateRangePicker() {
                    if (!$(element).data('daterangepicker')) {
                        ...
                        $(element).attr('placeholder', 'All Time');
